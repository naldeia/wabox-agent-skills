...
description: Integra a API de WhatsApp do Wabox (api.wabox.me) em qualquer código — conectar um número por QR/pairing, enviar texto/mídia/interativos, receber webhooks (received, delivery, message_status, instance_status, chat_presence), verificar a assinatura HMAC, tratar fila, erros e rate limit. Use quando o usuário pedir para "enviar WhatsApp pelo Wabox", "receber mensagens do Wabox", "criar canal de WhatsApp com Wabox", migrar do z-api ou de Baileys/whatsapp-web.js, ou sempre que aparecer uma URL /instances/{id}/token/{token}.
---
...
**Confirme a versão instalada antes de copiar exemplos.** As versões npm `@wabox/sdk@0.1.0`/`0.1.1` são anteriores às API keys do workspace: `createWabox` e os helpers de webhook funcionam, mas a opção é `clientToken` (não `apiKey`) e o cliente de provisionamento usa credenciais que a API não aceita mais. Para Account API/campos novos, siga [references/sdk-compatibility.md](references/sdk-compatibility.md): use HTTP conforme o OpenAPI atual enquanto a versão instalada não incluir esses recursos. Não use `apiKey`/`createWaboxAccount({ apiKey })` só porque existe no código-fonte/docs.

## Fluxo padrão de integração

1. **Credenciais**: `instance_id` e `token` vêm do painel ou da Account API (skill `wabox-partner`). Para um número, use variáveis de ambiente; em um SaaS, guarde as credenciais protegidas por canal e o vínculo com o tenant. O token na URL é a credencial base e, por padrão, nenhum header é necessário. Se o workspace ligar **"Exigir API key nas rotas de instância"** (Segurança › API keys), envie também uma API key do workspace com a permissão `instances:operate`: `Authorization: Bearer wbx_key_…` (`apiKey` no SDK; variável sugerida `WABOX_API_KEY`). Sem ela → `401 api_key_required`; key sem a permissão → `403 insufficient_scope`. Migrando do z-api: o header `Client-Token` continua aceito nas rotas de instância como alias — basta colocar nele uma API key do Wabox (Client-Tokens gerados antes de 2026-09-21 viraram a key "Client-Token (migrado)" e seguem válidos nesse header). Nunca exponha tokens nem API keys no front-end nem em logs (a URL contém o token — mascare).
2. **Conectar**: `GET /status` → se `status: "qr"`, mostre `GET /qr-code` (`value` = data URL PNG, rotaciona a cada ~20 s) ou `GET /phone-code/{phone}` (código de pareamento). Prefira o webhook `instance_status` (`status: "qr"` = hora de mostrar o QR, o código não vem no evento; `status: "connected"` = pareado) a fazer polling; se precisar, consulte a cada 10 s.
3. **Webhooks**: configure antes de parear. `PUT /webhooks` com `received_url`, `delivery_url`, `message_status_url`, `instance_status_url`, `chat_presence_url` — ou `{ "single_url_enabled": true, "single_url": "https://..." }`. Definir uma URL na instância desliga `use_workspace_webhooks`. Com configuração própria, use o `secret` de `GET /webhooks`; com herança, use o de `GET /account/webhooks` (o GET da instância mostra os campos próprios, não os efetivos). **Verifique `X-Wabox-Signature` sobre o corpo cru** e roteie pelo `type` e `instance_id` do corpo verificado.
...
- `scripts/wabox.sh GET status` · `scripts/wabox.sh POST send-text '{"phone":"55…","message":"oi"}'` — curl com `WABOX_INSTANCE_ID`/`WABOX_TOKEN` do ambiente (`WABOX_API_URL` para outro host; `WABOX_API_KEY` se o workspace exige API key nas rotas de instância).
